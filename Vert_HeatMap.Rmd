...
aves_chis_combine <- aves_chis %>%
  ...
  select(id, source, month, year, island, class, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, type, family, genus, scientificName, locality, institutionCode) %>%
...
all_data_coords_aves = anti_join(all_data_coords_aves_unclean, aves_coord_remove)
all_data_coord_aves_removedlist = semi_join(all_data_coords_aves_unclean, aves_coord_remove)

aves_removelist = all_data_coord_aves_removedlist %>% add_count(latitude, longitude, locality, institutionCode, year, sort=TRUE) %>% select(latitude, longitude, locality, institutionCode, year, n)
aves_removelist2 = distinct(aves_removelist, .keep_all=TRUE)
write_csv(aves_removelist2, "aves/avesGraphs/aves_removelist2.csv")
```
...
amph_chis_combine <- amph_chis %>%
  ...
  dplyr::select(id, month, year, island...128, class, acceptedGenus, acceptedName, latitude, longitude, locality, institutionCode) %>%
  rename(island = "island...128") %>%
  add_column(source="Cal-IBIS", type="Collection")

```

```{r Cleaning up data Amphibians}
# create a dataframe of all complete cases of name, lat/long, island, and year with unique lat/long and locality to check, made a list of ones to remove from excel sheet
...
amph_latlong = amph_data %>% select(id, island, latlong, latitude, longitude, locality, year, institutionCode)
...
all_data_coords_amph = anti_join(all_data_coords_amph_unclean, amph_coord_remove)
all_data_coord_amph_removedlist = semi_join(all_data_coords_amph_unclean, amph_coord_remove)

amph_removelist = all_data_coord_amph_removedlist %>% add_count(latitude, longitude, locality, institutionCode, year, sort=TRUE) %>% select(latitude, longitude, locality, institutionCode, year, n)
amph_removelist2 = distinct(amph_removelist, .keep_all=TRUE)
write_csv(amph_removelist2, "amphibian/amphibianGraphs/amph_removelist2.csv")
```
...
all_data_coords <- all_data_coords_amph %>%
  dplyr::filter(!is.na(latitude), !is.na(longitude))
...
for(i in 1:8){
	...
	coll.Summary[i,"mnRich"] <- mean(rich.cel[rich.cel!=0])
	coll.Summary[i,"mnColl"] <- mean(coll.cel[rich.cel!=0])
	coll.Summary[i,"totRich"] <- length(unique(occ$acceptedName[occ$island==isl.names[i]]))
	coll.Summary[i,"mnRedund"] <- round(mean(1-(rich.cel[rich.cel!=0]/coll.cel[rich.cel!=0])),3)
	coll.Summary[i,"mnRedundwE"] <- round(1/mean((coll.cel-rich.cel)),3)
}
...
sum_table <- as.data.frame(coll.Summary) %>%
  rownames_to_column(var = "island") %>% 
  rename("Island Collections" ="totColls", "1 km cells" ="totCells", "Empty 1 km cells" ="Empty", "Mean records/cell"="mnColl", "Mean redundancy" ="mnRedundwE", "% empty cells"="perEmpty", "Island Richness"="totRich", "Mean richness/cell"="mnRich") %>%
  relocate("island", "Island Collections", "1 km cells", "Empty 1 km cells", "% empty cells", "Mean records/cell", "Mean redundancy" )
...
png("amph_anacapa.png", width=300, height=250, res=300)
dev.off()
...
#rosa
i=3
ro=plot(coll.rast, col=colRamp, xlim=c(-25000,5000), ylim=c(-470000,-430000),
     xaxt = "n", yaxt = "n") +
  plot(is.map, add=T) +
  points(occ$Lon_alb,occ$Lat_alb, cex=0.3, col="blue", pch=19) 
#miguel
i=4
mi=plot(coll.rast, col=colRamp,xlim=c(-45000,-25000),ylim=c(-450000,-430000),
     xaxt = "n", yaxt = "n") +
  plot(is.map, add=T) +
  points(occ$Lon_alb,occ$Lat_alb, cex=0.3, col="blue", pch=19)  
#catalina
i=5
ca=plot(coll.rast, col=colRamp,xlim=c(125000,165000),ylim=c(-530000,-500000),
     xaxt = "n", yaxt = "n") +
  plot(is.map, add=T) +
  points(occ$Lon_alb,occ$Lat_alb, cex=0.3, col="blue", pch=19)  
...
rep_latlong = rep_data %>% select(id, island, latlong, latitude, longitude, locality, year, institutionCode)
...
write_csv(repcoord3, "coordinates/rep_coordinates_locality.csv")

#download list of ones to remove
rep_coord_remove = read_csv("coordinates/rep_coordinates_locality.csv")

all_data_coords_rep = anti_join(all_data_coords_rep_unclean, rep_coord_remove)
all_data_coord_rep_removedlist = semi_join(all_data_coords_rep_unclean, rep_coord_remove)

rep_removelist = all_data_coord_rep_removedlist %>% add_count(latitude, longitude, locality, institutionCode, year, sort=TRUE) %>% select(latitude, longitude, locality, institutionCode, year, n)
rep_removelist2 = distinct(rep_removelist, .keep_all=TRUE)
write_csv(rep_removelist2, "reptile/reptileGraphs/rep_removelist2.csv")
...
mam_latlong = mam_data %>% select(id, island, latlong, latitude, longitude, locality, year, institutionCode)
...
all_data_coords_mam = anti_join(all_data_coords_mam_unclean, mam_coord_remove)
all_data_coord_mam_removedlist = semi_join(all_data_coords_mam_unclean, mam_coord_remove)


mam_removelist = all_data_coord_mam_removedlist %>% add_count(latitude, longitude, locality, institutionCode, year, sort=TRUE) %>% select(latitude, longitude, locality, institutionCode, year, n)
mam_removelist2 = distinct(mam_removelist, .keep_all=TRUE)
write_csv(mam_removelist2, "mammal/mammalGraphs/mam_removelist2.csv")
```
...
all_data_coords <- all_data_coords_mam %>%
  dplyr::filter(!is.na(latitude), !is.na(longitude))
