...
```{r Creating a new datasheet with just endemics}
rep_end <- read_csv("reptile/rept_end.csv")
...
```{r Creating a new datasheet with just endemics}
amph_end <- read_csv("amphibian/amphib_end.csv")
...
salamander_yearrare = ggplot(amph_end_join, aes(x=year, fill=source)) + 
  geom_bar() + theme_bw() + labs(x="Year", y ="Number of Data Points") +   scale_fill_brewer(palette = "Dark2") + facet_wrap(vars(acceptedName), nrow=1, scales="free") + scale_y_continuous(expand=expansion(mult=c(0,0.05))) + scale_x_continuous(breaks=c(1880, 1900, 1920, 1940, 1960, 1980, 2000, 2020)) + theme(axis.text = element_text(size = 8), axis.title = element_text(size=10), strip.text = element_text(size=11)) 
