...
#BIRDS
```{r bird data}
aves_end = read_csv("aves/aves_end.csv")
...
aves_sbmnh_obs = read_csv("aves/aves_sbmnh_obs2_09082021.csv")
aves_sbmnh_col = read_csv("aves/aves_sbmnh_col_09082021.csv")
...
aves_end <- read_csv("aves/aves_end2.csv")
...
# Rynhops niger, changed it csv. now onlt diff by Circus cyaneus, which I think was meant to be local C. hudsonius bc C. cyaneus is native to Eurasia, just added a new row with it to csv, check with DAK
```
...
```{r Creating non-endemic rare only graphs}
#FIX 
aves_rare = filter(aves_end_join,x =="Branta") 
...
```{r rare seabird temporal }
aves_rare_seabird = filter(aves_end_join, group=="2") %>%
    mutate(source2 = fct_relevel(source, "Cal-IBIS", "iNaturalist", "EBIRD", "SBMNH Collections", "SBMNH Observations"))

#temporal
aves_seabird_year = ggplot(aves_rare_seabird, aes(x=year, fill=source2)) + 
  geom_bar(width=0.5) + 
  theme_bw() + 
  labs(x="Year", y ="Number of Data Points", fill="source") +   scale_fill_brewer(palette = "Dark2") + facet_wrap(vars(acceptedName), nrow=5, scales="free")  + scale_y_continuous(expand=expansion(mult=c(0,0.05))) + scale_x_continuous(breaks=c(1820, 1840, 1860, 1880, 1900, 1920, 1940, 1960, 1980, 2000, 2020)) + theme(axis.text = element_text(size = 8), axis.title = element_text(size=15), strip.text = element_text(size=14), legend.text = element_text(size=12)) 
aves_seabird_year

ggsave(plot=aves_seabird_year, filename="aves/AvesGraphs/aves_seabird_year.png", height=50, width=45, units="cm", dpi=300)
```
```{r rare seabird island}

aves_seabird_islandsum <- aves_rare_seabird %>%
  mutate(no_sample = 1) %>%
  group_by(island, acceptedName, source2) %>%
  summarise(samples = sum(no_sample))

aves_seabird_islandsum2 <- na.omit(subset(aves_seabird_islandsum, select = c(island, samples, source2, acceptedName)))

aves_seabird_island <- ggplot(data = aves_seabird_islandsum2, aes(x = samples, y = fct_rev(island), fill=source2)) + geom_bar(stat = "identity") + facet_wrap(vars(acceptedName), nrow=5, scales="free_x") +
  labs(y ="Island",  x = "Number of Data Points", fill = "source") + theme_bw() +  scale_fill_brewer(palette="Dark2") + scale_x_continuous(expand=expansion(mult=c(0,0.5))) + theme(axis.text = element_text(size = 8), axis.title = element_text(size=15), strip.text = element_text(size=14), legend.text = element_text(size=12)) 

aves_seabird_island

ggsave(plot=aves_seabird_island, filename="aves/AvesGraphs/aves_seabird_island.png", height=50, width=45, units="cm", dpi=300)

```

```{r rare bird of prey temporal }
aves_rare_predator = filter(aves_end_join, group=="3") %>%
    mutate(source2 = fct_relevel(source, "Cal-IBIS", "iNaturalist", "EBIRD", "SBMNH Collections", "SBMNH Observations"))

#temporal
aves_predator_year = ggplot(aves_rare_predator, aes(x=year, fill=source2)) + 
  geom_bar(width=0.7) + 
  theme_bw() + 
  labs(x="Year", y ="Number of Data Points", fill="source") +   scale_fill_brewer(palette = "Dark2") + facet_wrap(vars(acceptedName), nrow=4, scales="free_y")  + scale_y_continuous(expand=expansion(mult=c(0,0.05))) + scale_x_continuous(limits=c(1850, 2022), breaks=c(1820, 1840, 1860, 1880, 1900, 1920, 1940, 1960, 1980, 2000, 2020)) + theme(axis.text = element_text(size = 8), axis.title = element_text(size=15), strip.text = element_text(size=14), legend.text = element_text(size=12)) 
aves_predator_year

ggsave(plot=aves_predator_year, filename="aves/AvesGraphs/aves_predator_year2.png", height=40, width=40, units="cm", dpi=300)
```
```{r rare bird of prey island}

aves_predator_islandsum <- aves_rare_predator %>%
  mutate(no_sample = 1) %>%
  group_by(island, acceptedName, source2) %>%
  summarise(samples = sum(no_sample))

aves_predator_islandsum2 <- na.omit(subset(aves_predator_islandsum, select = c(island, samples, source2, acceptedName)))

aves_predator_island <- ggplot(data = aves_predator_islandsum2, aes(x = samples, y = fct_rev(island), fill=source2)) + geom_bar(stat = "identity") + facet_wrap(vars(acceptedName), nrow=4, scales="free_x") +
  labs(y ="Island",  x = "Number of Data Points", fill = "source") + theme_bw() +  scale_fill_brewer(palette="Dark2") + scale_x_continuous(expand=expansion(mult=c(0,0.5))) + theme(axis.text = element_text(size = 8), axis.title = element_text(size=15), strip.text = element_text(size=14), legend.text = element_text(size=12)) 

aves_predator_island

ggsave(plot=aves_predator_island, filename="aves/AvesGraphs/aves_seabird_island.png", height=40, width=30, units="cm", dpi=300)
