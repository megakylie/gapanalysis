...
sqshrew_yearrare = ggplot(sqshrew, aes(x=year, fill=source)) + 
  geom_bar(width=0.75) + 
  theme_bw() + 
  labs(x="Year", y ="Number of Occurences") +   scale_fill_brewer(palette = "Dark2") + facet_wrap(vars(acceptedName), nrow=1)  + scale_y_continuous(expand=expansion(mult=c(0,0.05))) + scale_x_continuous(limits=c(1920, 2020), breaks=c(1890, 1900, 1920, 1940, 1960, 1980, 2000, 2020))
...
mammal_end_island2$acceptedName[is.na(mammal_end_island2$acceptedName)] <- "Antrozous pallidus pacificus"


mam_end_island_ex <- ggplot(data = mammal_end_island2, aes(x = samples, y = fct_rev(island), fill=source)) + geom_bar(stat = "identity") + facet_wrap(vars(acceptedName), ncol=2, scales="free_x") +
  labs(y ="Island", 
