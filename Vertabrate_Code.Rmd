...
#print for Denise 
mam_chis_dak <- mam_chis %>%
  rename(latitude = "decimalLatitude", longitude = "decimalLongitude") %>%
  select(source, gbifID, institutionCode, catalogNumber, recordedBy, month, year, island, locality, class, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, CollectionType, preparations)
write_csv(mam_chis_dak, "mam_chis_dak.csv", na="")

mam_instit = table(mam_chis_dak$institutionCode)
mam_instit_table = as.data.frame(mam_instit) %>% rename(Institution = "Var1", n = "Freq" ) %>% arrange(desc(n))
write_csv(mam_instit_table, "extratables/mam_instit.csv")
...
mam_coll <- ggplot(mam_chis2, aes(x=CollectionType, fill=CollectionType)) + geom_bar() + theme_bw() + theme(legend.position = "none") + theme(axis.text.x = element_text(angle =90)) + labs(x="Collection Type", y="Number of Mammal Specimens") + stat_count(geom = "text", aes(label = stat(count)))
...
rep_inat_combine <- rep_inat %>%
  ...
  select(source, month, year, island, class, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude) %>% add_column(institutionCode="iNaturalist")

rep_chis_combine <- rep_chis %>%
  rename(latitude = "decimalLatitude", longitude="decimalLongitude") %>%
  select(source...60, month, year, island...122, class, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, institutionCode) %>%
  rename(island="island...122", source = "source...60")
...
#print for Denise 
rep_instit = table(rep_chis_combine$institutionCode)
rep_instit_table = as.data.frame(rep_instit) %>% rename(Institution = "Var1", n = "Freq" ) %>% arrange(desc(n))
write_csv(rep_instit_table, "extratables/rep_instit.csv")
...
rep_collgraph <- ggplot(rep_coll, aes(x=collectionType, fill=collectionType)) + geom_bar() + theme_bw() + theme(legend.position = "none") + labs(x="Collection Type", y="Number of Reptile Specimens") + scale_y_continuous(expand=expansion(mult=c(0,0.05))) + stat_count(geom = "text", aes(label = stat(count)))
...
amph_inat_combine <- amph_inat %>%
  ...
  add_column(type="Observation", institutionCode="iNaturalist")

amph_chis_combine <- amph_chis %>%
  rename(latitude = "decimalLatitude", longitude="decimalLongitude", month="month_", year="year_") %>%
  select( month, year, island...128, class, acceptedGenus, acceptedName, latitude, longitude, institutionCode) %>%
...
write.csv(amph_master, "Amphibian/AmphibianGraphs/amph_master.csv")


#print for Denise 
amph_instit = table(amph_chis_combine$institutionCode)
amph_instit_table = as.data.frame(amph_instit) %>% rename(Institution = "Var1", n = "Freq" ) %>% arrange(desc(n))
write_csv(amph_instit_table, "extratables/amph_instit.csv")
...
```{r Amph Collection Type}
amph_coll <-na.omit(subset(amph_chis, select=c(collectionType, order_, acceptedGenus, acceptedName)))
amph_collgraph <- ggplot(amph_coll, aes(x=collectionType, fill=collectionType)) + geom_bar() + theme_bw() + theme(legend.position = "none") + labs(x="Collection Type", y="Number of Amphibian Specimens") + scale_y_continuous(expand=expansion(mult=c(0,0.05))) + stat_count(geom = "text", aes(label = stat(count)))
