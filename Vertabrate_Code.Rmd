...
mam_coll <- ggplot(mam_chis2, aes(x=CollectionType, fill=CollectionType)) + geom_bar() + theme_bw() + theme(legend.position = "none") + theme(axis.text.x = element_text(angle = 90))
...
rep_island <- ggplot(data = rep_islandsum, aes(x = samples, y = fct_rev(island), fill = source)) + 
  geom_bar(stat = "identity") +
  labs(y ="Island", 
       x = "Number of Reptile Specimens Sampled",
       fill = "Data Source") +
  theme_bw() + 
...
rep_island_prop <- ggplot(data = rep_islandprop, aes(x = by_area, y = fct_rev(island), fill = source)) + 
  geom_bar(stat = "identity") +
  labs(y ="Island", 
       x = "Reptile Specimens Sampled/Island Area (km^2)",
       fill = "Data Source") +
  theme_bw() + 
...
rep_year = ggplot(rep_year, aes(x=year, fill=source)) + 
  ...
  labs(x="Year", y ="Number of Reptile Specimens Sampled") +   scale_fill_brewer(palette = "Dark2")
...
rep_ismonth <- ggplot(rep_month, aes(x=month, fill=month)) + 
  ...
  theme_bw() + 
  scale_y_continuous(expand=c(0,0.5)) + 
  labs(x="Reptile Sampling per Island by Month", y ="Number of Specimens Sampled") +  
  facet_wrap(vars(island), nrow=2, scales="free_y")
...
amph_island <- ggplot(data = amph_islandsum2, aes(x = samples, y = fct_rev(island), fill = source)) + 
  geom_bar(stat = "identity") +
  labs(y ="Island", 
       x = "Number of Amphibian Specimens Sampled",
       fill = "Data Source") +
  theme_bw() + 
...
amph_island_prop <- ggplot(data = amph_islandprop, aes(x = by_area, y = fct_rev(island), fill = source)) + 
  geom_bar(stat = "identity") +
  labs(y ="Island", 
       x = "Amphibian Specimens Sampled/Island Area (km^2)",
       fill = "Data Source") +
  theme_bw() + 
...
amph_yearplot = ggplot(amph_year, aes(x=year, fill=source)) + 
  ...
  labs(x="Year", y ="Number of Amphibian Specimens Sampled") +   scale_fill_brewer(palette = "Dark2")
...
amph_ismonth <- ggplot(amph_month, aes(x=month, fill=month)) + 
  ...
  theme_bw() + 
  scale_y_continuous(expand=c(0,0.5)) + 
  labs(x="Amphibian Sampling per Island by Month", y ="Number of Specimens Sampled") +  
  facet_wrap(vars(island), nrow=2, scales="free_y")
...
aves_chis <- read_csv("aves/aves_chis.csv")
aves_sbmnh <- read_csv("aves/aves_sbmnh.csv")
...
aves_inat_combine <- aves_inat %>%
  rename(class="iconic_tax") %>%
  select(source, month, year, island, class, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, collectionType)

aves_chis_combine <- aves_chis %>%
  rename(latitude = "decimalLatitude", longitude="decimalLongitude", year="year_", month="month_") %>%
  select(source, month, year, island, class, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, collectionType)

aves_sbmnh_combine <- aves_sbmnh %>%
  rename(latitude = "decimalLatitude", longitude="decimalLongitude") %>%
  select(source, month, year, island, class, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, collectionType)

aves_ebird_combine <- aves_ebird %>%
  rename(latitude = "decimalLatitude", longitude="decimalLongitude", year="year_", month="month_", collectionType = "collection Type") %>%
  select(source, month, year, island, class, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, collectionType)
  
aves_master = rbind(aves_inat_combine, aves_chis_combine, aves_sbmnh_combine, aves_ebird_combine) %>%
  # reorder the island names to show up N -> S on list
  mutate(island = fct_relevel(island, "San Miguel", "Santa Rosa", "Santa Cruz", "Anacapa", "Santa Barbara", "Santa Catalina", "San Nicolas", "San Clemente"))
## 342351 records!!! 
```
...
filter(!is.na(island)) %>%
   ungroup() %>%
  mutate(source2 = fct_relevel(source, "Cal-IBIS", "iNaturalist", "EBIRD", "SBMNH"))
...
aves_island <- ggplot(data = aves_islandsum, aes(x = samples, y = fct_rev(island), fill = source2)) + 
  geom_bar(stat = "identity") +
  labs(y ="Island", 
       x = "Number of Bird Specimens Sampled",
       fill = "Data Source") +
  theme_bw() + 
...
aves_island_prop <- ggplot(data = aves_islandprop, aes(x = by_area, y = fct_rev(island), fill = source2)) + 
  geom_bar(stat = "identity") +
  labs(y ="Island", 
       x = "Bird Specimens Sampled/Island Area (km^2)",
       fill = "Data Source") +
  theme_bw() + 
...
### Temporal Graph
```{r Graphing by year}
aves_year <- na.omit(subset(aves_master, select = c(island, year, source))) %>%
  mutate(source2 = fct_relevel(source, "Cal-IBIS", "iNaturalist", "EBIRD", "SBMNH"))
aves_yeargraph = ggplot(aves_year, aes(x=year, fill=source2)) + 
  geom_bar() + 
  theme_bw() + 
  scale_y_continuous(expand=c(0,0), limits = c(0, 9000)) + 
  scale_x_continuous(expand=c(0,0), limits=c(1810, 2021), breaks=c(1818, 1850, 1880, 1900, 1920, 1940, 1960, 1980, 2000, 2020)) + 
  labs(x="Year", y ="Number of Bird Specimens Sampled") +   scale_fill_brewer(palette = "Dark2")
...
aves_yeargraph2 = ggplot(aves_year, aes(x=year, fill=source)) + 
  ...
  scale_x_continuous(expand=c(0,0), limits=c(1810, 2021), breaks=c(1818, 1850, 1880, 1900, 1920, 1940, 1960, 1980, 2000, 2020)) + 
  labs(x="Year", y="Number of Bird Specimens Sampled (log scaled") +   scale_fill_brewer(palette = "Dark2")
...
aves_ismonth <- ggplot(aves_month, aes(x=month, fill=month)) + 
  ...
  theme_bw() + 
  scale_y_continuous(expand=c(0,0.5)) + 
  labs(x="Bird Sampling per Island by Month", y ="Number of Specimens Sampled") +  
  facet_wrap(vars(island), nrow=2, scales="free_y")
