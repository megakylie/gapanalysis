...
col_coord_remove = read_csv("coordinates/col_coordinates_remove2.csv")

all_data_coords_col = anti_join(all_data_coords_col_unclean, col_coord_remove, by=c("latitude", "longitude", "locality"))
all_data_coord_col_removedlist = semi_join(all_data_coords_col_unclean, col_coord_remove)

col_removelist = all_data_coord_col_removedlist %>% add_count(latitude, longitude, locality, institution, year, sort=TRUE) %>% select(latitude, longitude, locality, institution, year, n)
col_removelist2 = distinct(col_removelist, .keep_all=TRUE)
write_csv(col_removelist2, "coleoptera/coleopteraGraphs/col_removelist2.csv")
```




```{r Cleaning up coleoptera data with R code}
# create a dataframe of all complete cases of name, lat/long, island, and year

all_data_coords <- all_data_coords_col %>%
...
```
Please go the Grid Maps in the tabel of contents
...
```{r Cleaning up data Diptera locality info }
# create a dataframe of all complete cases of name, lat/long, island, and year with unique lat/long and locality to check, made a list of ones to remove from excel sheet
...
write_csv(dipcoord3, "coordinates/dip_coordinates_locality.csv")
...
```{r Cleaning up Diptera data with code}
# create a dataframe of all complete cases of name, lat/long, island, and year
all_data_coords <- all_data_coords_dip_unclean %>%
  dplyr::filter(!is.na(latitude), !is.na(longitude))
...
#Hymenoptera
```{r import hymenoptera data }
library(readxl)
chis = read_csv("hymenoptera/hym_chis_clean.csv")
bohart = read_csv("hymenoptera/hym_bohart_clean.csv")


chis_column <- chis %>%
  select(source, month_arabic, day_arabic, year, island, order, family, genus, specificEp, scientificName, decimalLat, decimalLon, locality, institutio, recordedBy) %>% rename(institutionCode="institutio", latitude="decimalLat", longitude="decimalLon", Collector="recordedBy")
bohart_column <- bohart %>%
  select(source, month_arabic, day_arabic, year, island, order, family, genus, specificEp, scientificName, "Decimal Latitude", "Decimal Longitude", locality, Collector) %>%
  rename(latitude="Decimal Latitude", longitude="Decimal Longitude") %>% add_column(institutionCode="Bohart")

hym_coord_data=rbind(chis_column, bohart_column)


write.csv(hym_coord_data, "hymenoptera/HymenopteraGraphs/hym_coord_data.csv")

```


```{r Cleaning up data Hymenoptera by locality info }
# create a dataframe of all complete cases of name, lat/long, island, and year with unique lat/long and locality to check, made a list of ones to remove from excel sheet

all_data_coords_hym_unclean <- hym_coord_data %>%
  dplyr::filter(!is.na(latitude), !is.na(longitude)) %>% filter(latitude != 0 | is.na(latitude))

hym_data = all_data_coords_hym_unclean %>% 
  unite("latlong", latitude:longitude, remove=FALSE, sep="")

hymcoord= table(hym_data$latlong)
hymcoord = as.data.frame.table(hymcoord) %>% rename(latlong="Var1")

hym_latlong = hym_data %>% select(island, latlong, latitude, longitude, locality, year)
hymcoord2 = left_join(hymcoord, hym_latlong) %>% rename(locality_latlong ="latlong")
hymcoord3 = distinct(hymcoord2, across(contains("locality")), .keep_all=TRUE) %>% add_column(taxa="hymenoptera")

write_csv(hymcoord3, "coordinates/hym_coordinates_locality.csv")

#download list of ones to remove
hym_coord_remove = read_csv("coordinates/hym_coordinates_remove.csv")

all_data_coords_hym = anti_join(all_data_coords_hym_unclean, hym_coord_remove)
all_data_coord_hym_removedlist = semi_join(all_data_coords_hym_unclean, hym_coord_remove)

hym_removelist = all_data_coord_hym_removedlist %>% add_count(latitude, longitude, locality, institutionCode, year, sort=TRUE) %>% select(latitude, longitude, locality, institutionCode, year, n)
hym_removelist2 = distinct(hym_removelist, .keep_all=TRUE)
write_csv(hym_removelist2, "hymenoptera/hymenopteraGraphs/hym_removelist2.csv")
```




```{r Cleaning up hymenoptera data with code}
# create a dataframe of all complete cases of name, lat/long, island, and year

all_data_coords <- all_data_coords_hym %>%
  dplyr::filter(!is.na(latitude), !is.na(longitude))
...
write_csv(all_data_coords3, "Hymenoptera/hym_all_data_coords_clean.csv")
```

```{r grid data download Hymenoptera}
################# Grid maps ############
# Matrix indicating which cells from islandRaster correspond to which island
island.cells <- read.csv("Heatmap_files/islandCells.csv")
...
# Data
occ <- read_csv("Hymenoptera/hym_all_data_coords_clean.csv") 
...
for(i in 1:dim(taxGrid)[1]){
	spCells <- occ$cellID[occ$class==taxa[i]]
...
write.csv(sum_table, "hymenoptera/hym_sum_table.csv", na="")
```
