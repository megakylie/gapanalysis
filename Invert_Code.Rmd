...
dip_lacm_combine <- dip_lacm %>%
  ...
  unite("scientificName", c(Genus, Species), sep=" ", na.rm=TRUE) %>% add_column(type="Collection", institution="LACM")

dip_chis_combine <- dip_chis %>%
  rename(scientificName="scientific", latitude="decimalLat", longitude="decimalLon", institution="institutio") %>%
...
hem_chis_combine <- hem_chis %>%
  rename(latitude = "decimalLat", longitude="decimalLon", institution="institutio") %>%
  select(source, institution, month, year, island, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude) %>% filter_all(any_vars(!is.na(.))) %>% add_column(type="Collection")
...
hem_bold_combine <- hem_bold %>%
  ...
  add_column(month = NA, year= NA) %>% add_column(institution="iNaturalist", type="Collection")
...
hym_chis_combine <- hym_chis2 %>%
  rename(latitude = "decimalLat", longitude="decimalLon", institution="institutio", month="month_arabic", acceptedName="acceptedName.y") %>%
  select(source, institution, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude) %>%  filter_all(any_vars(!is.na(.))) %>% add_column(type="Collection")

## hym_master from below and filter to only bohart and chis 
hym_collection = hym_master %>% filter(source %in% c("Bohart", "LACM")) %>% select(source, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName) %>% add_column(latitude="not_transferred", longitude = "not_transferred", type="Collection")
hym_collection$institution = hym_collection$source

hym_collection_master = rbind(hym_chis_combine, hym_collection) %>% add_column(phylum="Arthropoda", class="Insecta")
write.csv(hym_collection_master, "master/hym_collection_master.csv")
```
...
master_invert_1 = rbind(mol_master, chilo_master, diplo_master, bla_master, odo_master, phas_master, col_master, lep_master, arac_master, orth_master, derm_master, hem_master) %>% 
   select(source, institution, type, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude, phylum, class)
