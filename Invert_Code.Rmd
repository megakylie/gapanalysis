...
dip_inat_combine <- dip_inat %>%
  ...
  select(source, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude) %>% add_column(type="Observation", institution="iNaturalist")

dip_lacm_combine <- dip_lacm %>%
   select(source, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, Genus, Species, `Collector(s)`) %>% 
  add_column(latitude=NA, longitude=NA) %>%
  unite("scientificName", c(Genus, Species), sep=" ", na.rm=TRUE) %>% add_column(type="Collection", institution="LACM") %>% rename(Collectors="Collector(s)")

dip_chis_combine <- dip_chis %>%
  rename(scientificName="scientific", latitude="decimalLat", longitude="decimalLon", institution="institutio", Collectors="recordedBy") %>%
  select(source, institution, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude, Collectors) %>% add_column(type="Collection")

dip_nic_combine <- dip_nic %>%
  select(source, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, Latitude, Longitude, Collectors) %>% 
  add_column(scientificName =NA, institution = "SBBG Collected", type="Collection") %>% rename(latitude="Latitude", longitude="Longitude")

dip_collections = rbind(dip_chis_combine, dip_lacm_combine, dip_nic_combine)
write_csv(dip_collections, "master/dip_collections.csv", na="")

dip_coll_combine = dip_collections %>% select(source, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude, type, institution)

dip_master = rbind(dip_inat_combine, dip_coll_combine ) %>%
...
```{r download data}
#downloading the inaturalist and cal-ibis data
...
mol_chis <- read_csv("mollusks/mol_chis_redo.csv") %>%
  rename(scientificName = "scientific")

# mollusks synonymy 
mol_syn <- read_csv("mollusks/mol_synlist.csv")
mol_sea <- read_csv("mollusks/mol_sea.csv")

mol_chis2 = left_join(mol_chis, mol_syn, by=c("scientificName"))
mol_inat2 = left_join(mol_inat, mol_syn, by=c("scientificName"))

#remove sea animals first 
mol_remove <- paste(mol_sea, collapse="|")
mol_remove
   mol_chis3 <- mol_chis2 %>%  rename(acceptedName="acceptedName.y", acceptedGenus="acceptedGenus.y", acceptedFamily= "acceptedFamily.y") %>%
        filter(acceptedFamily != "Anabathridae" | is.na(acceptedFamily),
           acceptedFamily != "Areneidae" | is.na(acceptedFamily), 
           acceptedFamily != "Borsoniidae" | is.na(acceptedFamily), 
           acceptedFamily != "Cadlinidae" | is.na(acceptedFamily), 
           acceptedFamily != "Clathurellidae" | is.na(acceptedFamily),
           acceptedFamily != "Colloniidae" | is.na(acceptedFamily),
           acceptedFamily != "Creseidae" | is.na(acceptedFamily),
           acceptedFamily != "Dialidae" | is.na(acceptedFamily),
           acceptedFamily != "Enteroctopodidae" | is.na(acceptedFamily),
           acceptedFamily != "Eucyclidae" | is.na(acceptedFamily),
           acceptedFamily != "Flabellinopsidae" | is.na(acceptedFamily),
           acceptedFamily != "Lepidochitonidae" | is.na(acceptedFamily),
           acceptedFamily != "Mangeliidae" | is.na(acceptedFamily),
           acceptedFamily != "Margaritidae" | is.na(acceptedFamily),
           acceptedFamily != "Mitromorphidae" | is.na(acceptedFamily),
           acceptedFamily != "Newtoniellidae" | is.na(acceptedFamily),
           acceptedFamily != "Pleurobranchaeidae" | is.na(acceptedFamily),
           acceptedFamily != "Propeamussiidae" | is.na(acceptedFamily),
           acceptedFamily != "Rhizoridae" | is.na(acceptedFamily),
           acceptedFamily != "Solariellidae" | is.na(acceptedFamily),
           acceptedFamily != "Tegulidae" | is.na(acceptedFamily),
           acceptedFamily != "Tonicellidae" | is.na(acceptedFamily),
           acceptedFamily != "Tornatinidae" | is.na(acceptedFamily),
           acceptedFamily != "Xylophagaidae" | is.na(acceptedFamily),
           acceptedFamily != "Zebinidae" | is.na(acceptedFamily),
           acceptedFamily != "Solariellidae" | is.na(acceptedFamily),
           acceptedFamily != "Solariellidae" | is.na(acceptedFamily))
...
write_csv(mol_chis3, "mollusks/mol_chis3.csv")            
write_csv(mol_inat3, "mollusks/mol_inat3.csv")  
```


###Mol Step 2. Combine datasets 
```{r selecting and combining}
mol_inat3 <- read_csv("mollusks/mol_inat3.csv")
mol_chis3 <- read_csv("mollusks/mol_chis3.csv")

mol_inat_combine <- mol_inat3 %>%
  ...
  add_column(type="Observation", institution="iNaturalist")

mol_chis_combine <- mol_chis3 %>%
  rename(latitude = "decimalLat", longitude="decimalLon", institution="institutio") %>%
  select(source, institution, month, year, island, acceptedOrder, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, scientificName) %>%
  add_column(type="Collection")
...
mol_master = rbind(mol_inat_combine, mol_chis_combine) %>%
  ...
  filter(island != "Cedros" | is.na(island),
       island != "Coronados" | is.na(island),
       island != "Guadalupe" | is.na(island),
       island != "Todos Santos" | is.na(island))
...
#collections master for DAK
mol_collections = mol_chis3 %>% rename(latitude = "decimalLat", longitude="decimalLon", institution="institutio", Collectors="recordedBy", order="acceptedOrder") %>%
  select(source, institution, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, scientificName, Collectors) %>% add_column(type="Collection")
write.csv(mol_collections, "master/mol_collections.csv", na="")
```
...
mol_islandsum <- mol_master %>%
  mutate(no_sample = 1) %>%
  group_by(island, source2) %>%
  summarise(samples = sum(no_sample))
mol_islandsum <- na.omit(subset(mol_islandsum, select = c(island, samples, source2)))

mol_island <- ggplot(data = mol_islandsum, aes(x = samples, y = fct_rev(island), fill = source2)) + 
  geom_bar(stat = "identity") +
  labs(y ="Island", 
...
mol_island_prop <- ggplot(data = mol_islandprop, aes(x = by_area, y = fct_rev(island), fill = source2)) + 
  geom_bar(stat = "identity") +
  labs(y ="Island", 
...
mol_ismonth <- ggplot(mol_month, aes(x=month, fill=month)) + 
  geom_bar() + 
  theme_bw()  + scale_y_continuous(expand=expansion(mult=c(0,0.05)), breaks=function(x) unique(floor(pretty(seq(0, (max(x)+1 *1.1)))))) +
...
moltab_inat = table(mol_inat3$acceptedFamily)
moltab_inat = as.data.frame.table(moltab_inat)
moltab_inat <- moltab_inat %>%
  rename(iNaturalist = "Freq") 

moltab_chis = table(mol_chis3$acceptedFamily)
...
write.csv(mol_lit_complete, "mollusks/MolluskGraphs/mol_lit_complete.csv", na="")
...
chilo_inat_combine <- chilo_inat %>%
  ...
  rename(scientificName="scientific_name") %>% add_column(institution="iNaturalist", type="Observation")
...
diplo_chis_combine <- diplo_chis %>%
  rename(latitude = "decimalLatitude", longitude = "decimalLongitude", institution="institutionCode") %>%
  select(source, institution, month, year, island, acceptedOrder, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, scientificName) %>% add_column(type="Collection")
...
diplo_lacm_combine <- diplo_lacm %>%
  ...
  add_column(latitude = NA, longitude =NA) %>% add_column(institution="LACM", type="Collection")
...
#####master collections for DAK#####
diplo_chis_combine <- diplo_chis %>%
  rename(latitude = "decimalLatitude", longitude = "decimalLongitude", institution="institutionCode", Collectors="recordedBy") %>%
  select(source, institution, Collectors, month, year, island, acceptedOrder, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, scientificName) %>% add_column(type="Collection")

diplo_lacm_combine <- diplo_lacm %>% rename(Collectors="Collector") %>%
  select(source, month, year, island, acceptedOrder, acceptedFamily, acceptedGenus, acceptedName, Family, Collectors) %>% rename(scientificName= "Family") %>%
  add_column(latitude = NA, longitude =NA) %>% add_column(institution="LACM", type="Collection")


diplo_collection = rbind(diplo_chis_combine, diplo_lacm_combine) %>% rename(order="acceptedOrder")
write.csv(diplo_collection, "master/diplo_collection.csv", na="")
```
...
bla_inat_combine <- bla_inat %>%
  ...
  select(source, month, year, island, acceptedOrder, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude) %>% add_column(institution="iNaturalist", type="Observation")

bla_chis_combine <- bla_chis %>%
  rename(latitude = "decimalLatitude", longitude="decimalLongitude", institution="institutionCode") %>%
  select(source, institution, month, year, island, acceptedOrder, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude) %>% add_column(type="Collection")
...
#####master collections for DAK#####
bla_collection <- bla_chis %>%
  rename(latitude = "decimalLatitude", longitude = "decimalLongitude", institution="institutionCode", Collectors="recordedBy") %>%
  select(source, institution, Collectors, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, scientificName) %>% add_column(type="Collection")

write.csv(bla_collection, "master/bla_collections.csv", na="")
```
...
odo_inat_combine <- odo_inat %>%
  ...
  select(source, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude) %>% add_column(institution="iNaturalist", type="Observation")

odo_chis_combine <- odo_chis %>%
  rename(order = "order_", latitude = "decimalLat", longitude="decimalLon", scientificName="scientific", institution="institutio") %>%
  select(source, institution, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude) %>% add_column(type="Collection")
...
#####master collections for DAK#####
odo_collection <- odo_chis %>%
  rename(latitude = "decimalLat", longitude = "decimalLon", institution="institutio", Collectors="recordedBy", scientificName="scientific") %>%
  select(source, institution, Collectors, month, year, island, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, scientificName) %>% add_column(type="Collection", order="Odonata")

write.csv(odo_collection, "master/odo_collection.csv", na="")
...
phas_chis_combine <- phas_chis %>%
  ...
  select(source, institutio, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientific, latitude, longitude) %>%
  rename(scientificName ="scientific", institution="institutio") %>% add_column(type="Collection")
...
#####master collections for DAK#####
phas_collection <- phas_chis %>%
  rename(latitude = "decimalLat", longitude = "decimalLon", institution="institutio", Collectors="recordedBy", scientificName="scientific") %>%
  select(source, institution, Collectors, month, year, island, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, scientificName) %>% add_column(type="Collection", order="Phasmida")

write.csv(phas_collection, "master/phas_collection.csv", na="")
...
col_lacm_combine <- col_lacm %>%
  select(source, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName) %>% add_column(latitude = NA, longitude =NA) %>% add_column(institution="LACM", type="Collection")

col_master = rbind(col_chis_combine, col_inat_combine, col_lacm_combine) %>%
  # reorder the island names to show up N -> S on list
  mutate(island = fct_relevel(island, "San Miguel", "Santa Rosa", "Santa Cruz", "Anacapa", "Santa Barbara", "Santa Catalina", "San Nicolas", "San Clemente")) %>% add_column(phylum="Arthropoda", class="Insecta")
write.csv(col_master, "master/col_master.csv", na="")

##collections for DAK##
col_chis_coll <- col_chis %>%
  rename(order ="order_", latitude = "decimalLat", longitude="decimalLon", scientificName="scientific", institution="institutio", Collectors="recordedBy") %>%
  select(source, institution, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude, Collectors) %>% add_column(type="Collection")

col_lacm_coll <- col_lacm %>%
  rename(Collectors='Collector(s)') %>%
  select(source, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, Collectors) %>% add_column(latitude = NA, longitude =NA) %>% add_column(institution="LACM", type="Collection")

col_collection = rbind(col_chis_coll, col_lacm_coll)
write.csv(col_collection, "master/col_collection.csv", na="")
```
...
lep_lacm_combine <- lep_lacm %>%
  ...
  add_column(latitude=NA, longitude=NA, scientificName=NA) %>% add_column(institution="LACM", type="Collection")
...
##collections for DAK##
lep_chis_coll <- lep_chis %>%
  rename(order ="order_", latitude = "decimalLat", longitude="decimalLon", scientificName="scientific", institution="institutio", Collectors="recordedBy") %>%
  select(source, institution, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude, Collectors) %>% add_column(type="Collection")

lep_lacm_coll <- lep_lacm %>%
  rename(Collectors='Collector(s)') %>%
  select(source, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, Collectors) %>% add_column(latitude = NA, longitude =NA) %>% add_column(institution="LACM", type="Collection", scientificName="NA")

lep_collection = rbind(lep_chis_coll, lep_lacm_coll)
write.csv(lep_collection, "master/lep_collection.csv", na="")
...
arac_inat_combine <- arac_inat %>%
  ...
  select(source, month, year, island, class, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude) %>% add_column(institution="iNaturalist", type="Observation")

arac_chis_combine <- arac_chis %>%
  rename(order="order_", latitude = "decimalLat", longitude="decimalLon", scientificName="scientific", institution="institutio") %>%
  select(source, institution, month, year, island, class, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude) %>% add_column(type="Collection")


arac_lacm_combine <- arac_lacm %>%
  select(source, month, year, island, class, order, acceptedFamily, acceptedGenus, acceptedName) %>%
  add_column(latitude=NA, longitude =NA, scientificName="Scorpiones") %>% add_column(institution="LACM", type="Collection")
...
##collections for DAK##
arac_chis_coll <- arac_chis %>%
  rename(order ="order_", latitude = "decimalLat", longitude="decimalLon", scientificName="scientific", institution="institutio", Collectors="recordedBy") %>%
  select(source, institution, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude, Collectors) %>% add_column(type="Collection")

arac_lacm_coll <- arac_lacm %>%
  rename(Collectors='Collector(s)') %>%
  select(source, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, Collectors) %>% add_column(latitude = NA, longitude =NA) %>% add_column(institution="LACM", type="Collection", scientificName="Scorpiones")

arac_collection = rbind(arac_chis_coll, arac_lacm_coll)
write.csv(arac_collection, "master/arac_collection.csv", na="")
```
...
hem_inat_combine <- hem_inat %>%
  ...
  select(source, month, year, island, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude) %>% add_column(institution="iNaturalist", type="Observation")

hem_chis_combine <- hem_chis %>%
  rename(latitude = "decimalLat", longitude="decimalLon", institution="institutio") %>%
  select(institution, month, year, island, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude) %>% filter_all(any_vars(!is.na(.))) %>% add_column(type="Collection", source="Cal-IBIS")

hem_lacm_combine <- hem_lacm %>%
  select(month, year, island, acceptedFamily, acceptedGenus, acceptedName, scientificName) %>%
  add_column(source="LACM", latitude = NA, longitude = NA) %>% add_column(institution="LACM", type="Collection")

hem_bold_combine <- hem_bold %>%
  rename(latitude = "lat", longitude = "lon", institution="institution_storing") %>%
  select(source, island, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude) %>%
  add_column(month = NA, year= NA) %>% add_column(institution="BOLD", type="Collection")
...
write.csv(hem_master, "master/hem_master.csv", na="")

##collections for DAK##
hem_chis_coll <- hem_chis %>%
  rename( latitude = "decimalLat", longitude="decimalLon", institution="institutio", Collectors="recordedBy") %>%
  select(institution, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude, Collectors) %>% filter(rowSums(is.na(.))!=ncol(.)) %>% add_column(type="Collection", source="Cal-IBIS")

hem_lacm_coll <- hem_lacm %>%
  rename(Collectors='Collector(s)') %>%
  select(month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, Collectors) %>% add_column(latitude = NA, longitude =NA, source="LACM", institution="LACM", type="Collection")

hem_bold_coll <- hem_bold %>%
  rename(latitude = "lat", longitude = "lon", institution="institution_storing", Collectors="collectors") %>%
  select(source, island, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude, institution, Collectors) %>% add_column(month = NA, year= NA, order="Hemiptera", type="Collection")

hem_collection = rbind(hem_chis_coll, hem_lacm_coll, hem_bold_coll)
write.csv(hem_collection, "master/hem_collections.csv", na="")
```
...
orth_inat_combine <- orth_inat %>%
  select(month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude) %>%
    add_column(source="iNaturalist", institution="iNaturalist", type="Observation")

orth_chis_combine <- orth_chis %>%
  rename(latitude = "decimalLat", longitude="decimalLon", island="ArcMap_IslandNames", institution="institutio") %>%
  select(institution, month, year, island, order_, acceptedFamily, acceptedGenus, acceptedName...92, scientificName, latitude, longitude) %>%
...
orth_lacm_combine <- orth_lacm %>%
  ...
  add_column(latitude=NA, longitude=NA, scientificName=NA, source="LACM", institution="LACM", type="Collection")
...
##collections for DAK##
orth_chis_coll <- orth_chis %>%
    rename(latitude = "decimalLat", longitude="decimalLon", island="ArcMap_IslandNames", institution="institutio", Collectors="recordedBy") %>%select(institution, month, year, island, order_, acceptedFamily, acceptedGenus, acceptedName...92, scientificName, latitude, longitude, Collectors) %>%
  rename(order = "order_", acceptedName="acceptedName...92") %>% add_column(source="Cal-IBIS", type="Collection")

orth_lacm_coll <- orth_lacm %>%
  rename(Collectors='Collector(s)', island="Island") %>%
  select(month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, Collectors, scientificName) %>% add_column(latitude = NA, longitude =NA) %>% add_column(source ="LACM", institution="LACM", type="Collection")

orth_collection = rbind(orth_chis_coll, orth_lacm_coll)
write.csv(orth_collection, "master/orth_collection.csv", na="")
```
...
#
#Hymenoptera
IS A MESS because it isn't standardized with the other groups...I did it first...
...
hym_inat =read.csv("hymenoptera/hym_inat_clean.csv")
hym_lacm = read.csv("hymenoptera/hym_lacm_clean.csv")
hym_chis = read_csv("hymenoptera/hym_chis_clean.csv")
hym_bohart = read.csv("hymenoptera/hym_bohart_clean.csv")
bold = read.csv("hymenoptera/BOLD_IslandHymenoptera_Feb2021.csv")
```
```{r clean up datasets}
hym_syn= read_csv("hymenoptera/hym_synonymy_list.csv")
hym_chis2 = left_join(hym_chis, hym_syn, by=c("scientificName"))
hym_inat2 = left_join(hym_inat, hym_syn, by=c("scientificName"))
hym_lacm2 = left_join(hym_lacm, hym_syn, by=c("scientificName"))
hym_bohart2 = left_join(hym_bohart, hym_syn, by=c("scientificName"))

hym_chis_coll <- hym_chis2 %>%
  rename(latitude = "decimalLat", longitude="decimalLon", institution="institutio", month="month_arabic", acceptedName="acceptedName.y", Collectors="recordedBy") %>%
  select(source, institution, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude, Collectors) %>%  filter_all(any_vars(!is.na(.))) %>% add_column(type="Collection")

hym_lacm_coll <- hym_lacm2 %>% rename(month="month_arabic", Collectors="collector") %>% select(source, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, Collectors) %>% add_column(type="Collections", institution="LACM", latitude=NA, longitude=NA )

hym_bohart_coll <- hym_bohart2  %>% rename(month="month_arabic", Collectors="Collector", latitude="Decimal.Latitude", longitude="Decimal.Longitude") %>% select(source, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude, Collectors)%>% add_column(type="Collections", institution="Bohart")

## hym_master from below and filter to only bohart and chis 

hym_collection = rbind(hym_chis_coll, hym_lacm_coll, hym_bohart_coll)
write.csv(hym_collection, "master/hym_collection.csv", na="")
```
...
hym_synonymy_list = distinct(hym_master, scientificName, .keep_all=TRUE) %>% select(scientificName, acceptedFamily, acceptedGenus, acceptedName)
write_csv(hym_synonymy_list, "hymenoptera/hym_synonymy_list.csv")
...
derm_inat_combine <- derm_inat %>%
  select(month, year, island, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude) %>%
    add_column(source="iNaturalist", order="Dermaptera", institution="iNaturalist", type="Observation")

derm_chis_combine <- derm_chis %>%
  rename(latitude = "decimalLat", longitude="decimalLon", institution="institutio") %>%
  select(institution, month, year, island, order, acceptedFamily, acceptedGenus, acceptedName, scientificName, latitude, longitude) %>%
  add_column(source="Cal-IBIS", type="Collection")
...
#####master collections for DAK#####
derm_collection <- derm_chis %>%
  rename(latitude = "decimalLat", longitude = "decimalLon", institution="institutio", Collectors="recordedBy") %>%
  select(institution, Collectors, month, year, island, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, scientificName) %>% add_column(type="Collection", order="Dermaptera", source="Cal-IBIS")

write.csv(derm_collection, "master/derm_collection.csv", na="")
```
