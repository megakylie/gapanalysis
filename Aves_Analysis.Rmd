...
aves_master_syn = aves_master %>% distinct(scientificName, .keep_all=TRUE) %>% select(scientificName, acceptedFamily, acceptedGenus, acceptedName)
write.csv(aves_master_syn, "Aves/aves_master_syn.csv", na="")
```
...
aves_ebird <- read_csv("aves/aves_ebird_new2.csv")
...
#for Denise
aves_chis_dak <- aves_chis %>%
  rename(latitude = "decimalLatitude", longitude="decimalLongitude", year="year_", month="month_", type="collectionType") %>%
  select(id, source, institutionCode, catalogNumber, recordedBy, month, year, island, locality, class, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, type, family, genus, scientificName) %>%
  add_column(type2="Collection: Egg/Nest")

aves_sbmnh_col_dak <- aves_sbmnh_col %>%
  rename(latitude = "decimalLatitude", longitude="decimalLongitude", type="collectionType2") %>%
  select(id, source, institutionCode, catalogNumber, recordedBy, month, year, island, locality, class, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, type, family, genus, scientificName) %>%
  add_column(type2="Collection: Specimen")

aves_collections = rbind(aves_chis_dak, aves_sbmnh_col_dak)
write_csv(aves_collections, "aves_collections_dak.csv", na="")

aves_instit = table(aves_collections$institutionCode)
aves_instit_table = as.data.frame(aves_instit) %>% rename(Institution = "Var1", n = "Freq" ) %>% arrange(desc(n))
write_csv(aves_instit_table, "extratables/aves_instit.csv")
...
aves_inat_combine <- aves_inat %>%
  ...
  add_column(family= NA, genus =NA, type2="Observation", type3="Observation")
...
aves_sbmnh_obs_combine <- aves_sbmnh_obs %>%
  ...
  select(source, month, year, island, class, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, type, family, genus, scientificName) %>%
  add_column(type2="Observation", type3="Observation")
...
aves_ebird_combine <- aves_ebird %>%
  ...
  add_column(source="EBIRD", type="Observation", type2= NA, type3="Observation")
...
aves_master = aves_master %>% filter(!is.na(source)) 
aves_master_syn = read_csv("Aves/aves_master_syn.csv")
aves_master_syn2 = left_join(aves_master, aves_master_syn, by="scientificName") %>% rename(acceptedFamily="acceptedFamily.y", acceptedGenus="acceptedGenus.y", acceptedName="acceptedName.y") %>%
  select(source, month, year, island, class, acceptedFamily, acceptedGenus, acceptedName, latitude, longitude, type, family, genus, scientificName)
# Apply 
```
...
```{r}
write_csv(aves_master, "Aves/aves_master.csv")
...
aves_ebird_combine_ord <- aves_ebird %>%
  rename(latitude = "decimalLatitude", longitude="decimalLongitude") %>%
...
#filter out all NA rows 
aves_master_na <- filter(aves_master_na, rowSums(is.na(aves_master_na)) != ncol(aves_master_na)) # Apply 
write.csv(aves_master_na_ord, "master/aves_master.csv")
```
...
aves_island <- ggplot(data = aves_islandsum, aes(x = samples, y = fct_rev(island), fill = source2)) + 
  ...
  labs(y ="Island", 
       ...
       fill = "Data Source") +
  theme_bw() + 
  scale_x_continuous(limits =c(0,140000), expand = c(0,0), breaks=c(10000, 30000, 50000, 70000, 90000, 110000, 130000)) +
...
#by collection type
aves_master$type3 = as.character(aves_master$type3)
aves_islandsumty <- aves_master %>%
  mutate(no_sample = 1) %>%
  group_by(island, type3, type) %>%
  summarise(samples = sum(no_sample)) %>%
filter(!is.na(island)) 

aves_islandsumty <- na.omit(subset(aves_islandsumty, select = c(island, samples, type3, type)))
...
aves_islandtype <- ggplot(data = aves_islandsumty, aes(x = samples, y = fct_rev(island), fill=type)) + 
  ...
  labs(y ="Island", 
       x = "Number of Bird Data Points",
       fill = "Type") +
  theme_bw() + facet_wrap(vars(type3), nrow=1, scales="free") +
  scale_fill_brewer(palette = "Paired") + scale_x_continuous(expand=expansion(mult=c(0,0.2)))

aves_islandtype
ggsave(plot=aves_islandtype, filename="aves/AvesGraphs/aves_island_type.png", height=10, width=30, units="cm", dpi=300)
...
ggsave(plot=aves_island_prop_type, filename="aves/AvesGraphs/aves_island_prop_type.png", height=10, width=30, units="cm", dpi=300)
...
#collection type
dPalette <- c("#1B9E77", "#D95F02", "#E7298A", "#7FBC41")
aves_year2 <- na.omit(subset(aves_master, select = c(island, year, source, type2))) %>%
  mutate(source2 = fct_relevel(source, "Cal-IBIS (Egg Only)", "iNaturalist", "EBIRD", "SBMNH Collections", "SBMNH Observations"))
aves_yeargraph3 = ggplot(aves_year2, aes(x=year, fill=source2)) + 
  geom_bar(width=0.75) + 
  theme_bw() +   facet_wrap(vars(type2), nrow=3, scales="free") +
  scale_y_continuous(expand=expansion(mult=c(0,0.02))) + 
  scale_x_continuous(expand=c(0,0), n.breaks=10) +
  labs(x="Year", y="Number of Bird Data Points", fill="Data Source") + scale_fill_manual(values=dPalette)

aves_yeargraph3

ggsave(plot=aves_yeargraph3, filename="aves/AvesGraphs/aves_year_coll_noebird.png", height=25, width=40, units="cm", dpi=300)
...
aves_colltype = rbind(aves_sbmnh_col_combine, aves_chis_combine)
aves_colltype = na.omit(subset(aves_chis_combine, select = c(collectionType, source)))
aves_collgraph <- ggplot(aves_colltype, aes(x=type, fill=source)) + geom_bar() + theme_bw()  + labs(x="Collection Type", y="Number of Bird Specimens") + scale_y_continuous(expand=c(0,0), limits=c(0,6000), breaks=c(0, 1000, 2000, 3000, 4000, 5000, 6000))
...
avestab= table(aves_master_syn2$acceptedName)
avestab = as.data.frame.table(avestab) 
avestab2 = avestab %>%
  rename(acceptedName = "Var1", total = "Freq")

aves_master2 = aves_master_syn2 %>%
...
aves_phylo = read_csv("aves/NACC_list_species.csv") %>% rename(acceptedFamily="family", acceptedGenus="genus", acceptedName="species")




aves_table_phylo = full_join(aves_phylo, aves_table5) #can't use filter to get rid of taxa not in our data rows with a na filter because it reorders the rows! also why full join was used so the rows would stay in the phylogenetic order

aves_table_complete <-aves_table_phylo %>%
...
aves_phylo_R2 = read_csv("aves/aves_phylogenetic_onlytaxainourdata.csv") %>% rename(acceptedFamily="Family", acceptedGenus="Genus", acceptedName="Species")
avestable_combo_phylo = full_join(aves_phylo_R2, avestable_combo2)

aves_table_combo_complete <- avestable_combo_phylo %>%
   rename(Species = "acceptedName", Family = "acceptedFamily", Genus = "acceptedGenus") %>%
...
#brewer.pal(5, name="Dark2") Color codes for the brewer colors, need 1, 2 and 4 for this one
#[1] Cal-IBIS "#1B9E77" iNaturalist "#D95F02" "EBIRD #7570B3" SBMNH Col "#E7298A" SBMNH Obs "#7FBC41"
cPalette <- c("#E7298A", "#7FBC41")

aves_end_c = filter(aves_master, acceptedName=="Selasphorus sasin sedentarius")
aves_end_cc = filter(aves_end_c, type=="Collection: Specimen")
aves_end_co = filter(aves_end_c, source=="SBMNH Observations")
aves_end_c2 = rbind(aves_end_cc, aves_end_co)
#temporal
...
#Cal-IBIS "#1B9E77"            iNaturalist "#D95F02"            EBIRD #7570B3"
#SBMNH Col "#E7298A"            SBMNH Obs "#7FBC41"
jPalette <- c("#D95F02", "#7570B3", "#E7298A", "#7FBC41")
