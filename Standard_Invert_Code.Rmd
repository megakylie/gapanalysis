...
occmonth <- ggplot(lep_year, aes(x=month, fill=source)) + 
  geom_bar() + 
  theme_bw() + 
  scale_y_continuous(expand=c(0,0)) + 
  labs(x="Month", y ="Number of Lepidoptera Specimens Samples") +
  scale_x_continuous(breaks = seq(1, 12, by=1)) 
...
ggsave(plot=lep_ismonth, filename="lepidoptera/LepidopteraGraphs/lep_seasonal.png", height=5, width=9, units="in", dpi=300)
...
lep_table_complete <- lep_table_complete %>%
  mutate(Percentage = (Sum/9392))

library(reactable)
reactable(lep_table_complete)


write.csv(lep_table_complete, "lepidoptera/LepidopteraGraphs/lep_table_complete.csv", na="0")
```
#
#Arachnida 
