...
hym_islandsum <- mastersyn %>%
  ...
  group_by(island, source) %>%
  summarise(samples = sum(no_sample))
hym_islandsum <- na.omit(subset(hym_islandsum, select = c(island, samples, source)))
hym_islandsum = hym_islandsum[c(2:33),] 
hym_islandsum$source <- as.factor(hym_islandsum$source)
...
hym_year = ggplot(mastersyn, aes(x=year, fill=source)) + 
  geom_bar() + 
  theme_bw() + 
