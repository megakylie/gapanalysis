...
#New Graph Making code with JAL edits, standardized graph code!
## Island Graph 
```{r Graphing by island}
...
## this factor isn't working, some data is being removed??
hym_islandsum$source <- factor(hym_islandsum$source,                
                         levels = c("Cal-IBIS", "iNaturalist", "LACM", "Bohart"))
##

hym_island_plot <- ggplot(data = hym_islandsum, aes(x = samples, y = fct_rev(island), fill = source)) + 
  geom_bar(stat = "identity") +
  labs(x ="Island", 
...
ggsave(plot=hym_island_plot, filename="diptera/DipteraGraphs/hym_island.png", height=5, width=8, units="in", dpi=300)
...
hym_year = ggplot(hym_master_syn, aes(x=year, fill=source)) + 
  geom_bar() + 
  theme_bw() + 
...
# by month and island 
hym_master_syn$month <- as.factor(hym_master_syn$month)
hym_month <- na.omit(subset(hym_master_syn, select = c(island, month)))
hym_month$month <- factor(hym_month$month,                
...
ggsave(plot=hym_ismonth, filename="diptera/DipteraGraphs/hym_seasonal.png", height=5, width=9, units="in", dpi=300)
