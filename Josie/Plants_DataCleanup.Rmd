...
plants_raw_1 <- read_csv("Josie/Plants_Redownload/plants_raw_1.csv", col_types = cols(.default = "c", isl_name = "c")) 
plants_raw_2 <- read_csv("Josie/Plants_Redownload/plants_raw_2.csv", col_types = cols(.default = "c", isl_name = "c")) 
...
by_isl <- read_csv("Josie/byislsp.csv", col_types = cols(.default = "c")) %>%
  ...
  select(scientificName, assign_by_island)
...
kylie_syn <- read_csv("Josie/KJE_synonymylist.csv", col_types = cols(.default = "c")) %>%
  mutate(scientificName = str_to_lower(scientificName),
         acceptedName = str_to_lower(acceptedName))

elim_list <- read_csv("Josie/Plants_ElimList_Revised2.csv", col_types = cols(.default = "c")) %>%
...
elim_list_2 <- read_csv("Josie/Plants_ElimList_round3_2.csv", col_types = cols(.default = "c")) %>%
  mutate(Eliminate = replace_na(Eliminate, "Keep"),
...
plants_combo3 <- left_join(plants_combo2, elim_list_2) %>%
  ...
  filter(Eliminate != "Elim" | is.na(Eliminate)) %>%
  select(-SMI, -SCR, - A, -SR, -SBA, -SNI, -SCL, -SCA, -assign_by_island, -Eliminate)

# final clean
masterlist <- read_csv("Josie/PlantMasterList.csv", col_types = cols(.default = "c")) %>%
  ...
  select(-old_name, -count_in_dataset, -final_syn) %>%
  distinct()
...
plants_byisl_join <- left_join(plants_byisl, by_isl) %>%
  select(scientificName, new_datasetname, isl_name, SMI:SCA)
...
plants_keep <- plants_nonisl_clean %>% 
  ...
  select(kingdom_final, phylum_final, class_final, order_final, family_final, old_name, Sci_Name,
  ...
  group_by(kingdom_final, phylum_final, class_final, order_final, family_final, isl_name, Sci_Name) %>%
  summarise(count = n()) %>%
  spread(isl_name, count, fill = 0)

write_csv(plants_keep, "Josie/ForKristen2/Data_nobyisl.csv")

  group_by(final_kingdom, phylum, class, order, Family_4, scientificName, sciname_3, Eliminate, SMI,	SR,	SCR,	A,	SBA,	SNI,	SCL,	SCA) %>%
  summarize(count = n()) %>%
