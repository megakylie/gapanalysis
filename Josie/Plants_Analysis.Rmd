...
counties <- readOGR("Heatmap_files/Master_IslandNames.shp")
chis_counties <- counties[counties$NAME%in%c("Santa Rosa", "Santa Cruz", "San Miguel", "Anacapa", "San Clemente", "Santa Barbara", "Santa Catalina", "San Nicolas")]

# cropping parts of counties that are not of interest
is.map<-crop(chis_counties, extent(-1.5e+05,2.5e+05,-6e+05,-4e+05))
...
occ<-read.csv("Josie/Data/DraftData.csv")
...
ggplot(data = plants_islandsum, aes(y = samples, x = year, fill = isl_name)) + 
  ...
  scale_x_continuous(limits = c(1850,2021), expand = c(0,0)) +
