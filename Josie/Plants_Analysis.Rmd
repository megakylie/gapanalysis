...
spat_occ <- read_csv("Josie/Data/Data_SpatialClean_20211022.csv") 
...
# Load the cleaned specimen data, isolate islands of interest (CA 8)
occ <- read_csv("Josie/Data/Data_20211022.csv")%>%
  dplyr::filter(institutionCode != "iNaturalist")
...
spat_occ <- cbind(spat_occ,cellID); rm(cellID)
length(unique(spat_occ$cellID))

# Make a big grid
w <- which(!is.na(isl.rast[]))
taxa <- sort(unique(spat_occ$sci_name))
...
for(i in 1:dim(taxGrid)[1]){
	spCells <- spat_occ$cellID[spat_occ$sci_name==taxa[i]]
...
for(i in 1:8){
	...
	coll.Summary[i,"totRich"] <- length(unique(spat_occ$sci_name[spat_occ$isl_name==isl.names[i]]))
...
write_csv(sum_table, "Josie/ForMatt/SummaryTable.csv")
...
plot(rich.rast, col=colRamp, xlim=c(50000,61000), ylim=c(-450000,-440000),
     xaxt = "n", yaxt = "n") +
  plot(is.map, add=T) +
  points(spat_occ$Lon_alb,spat_occ$Lat_alb, cex=0.3, col="sienna3", pch=19) 
